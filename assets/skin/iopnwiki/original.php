...
<!-- Опен граф -->
    ...
    <?require("components/scripts/MathJax.php")?>
</head>
<body>
    <header>
                ...
                <span class="info">
                    ...
                    <div class="info-panel right" style="min-width: 250px; padding: 15px;">
                        <div style="font-size:16px; position:relative; margin: -10px 10px;">
                            <?require("components/render/render_meta_info.php")?>
                        </div>
                    </div>
                </span>
            </h1>
            <div id="Root">
                <?require("components/render/render_page.php")?>
            </div>
        </div>
        <div class="End"></div>
    </main>
    <script>
        const root = document.getElementById('Root'); //элемент роот
        if (root) {
            const firstChild = root.firstElementChild; //получения первого элемента
            if (firstChild && firstChild.tagName === 'H2') {root.removeChild(firstChild);} //проверка и  удаления
        }
    </script>
</body>
</html>